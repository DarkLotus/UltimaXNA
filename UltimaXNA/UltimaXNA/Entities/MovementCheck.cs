...
namespace UltimaXNA.Entities
{
    static class MovementCheck
    {
        ...
        private const OpenUO.Ultima.TileFlag ImpassableSurface = OpenUO.Ultima.TileFlag.Impassable | OpenUO.Ultima.TileFlag.Surface;
        ...
        private static bool IsOk(bool ignoreDoors, int ourZ, int ourTop, MapObjectStatic[] tiles, List<Item> items)
        {
            for (int i = 0; i < tiles.Length; ++i)
            {
                ...
                OpenUO.Ultima.ItemData itemData = Data.TileData.ItemData[check.ItemID & 0x3FFF];
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                OpenUO.Ultima.ItemData itemData = Data.TileData.ItemData[itemID];
                var flags = itemData.Flags;

                if ((flags & ImpassableSurface) != 0) // Impassable || Surface
                {
                    if (ignoreDoors && ((flags & OpenUO.Ultima.TileFlag.Door) != 0 || itemID == 0x692 || itemID == 0x846 || itemID == 0x873 || (itemID >= 0x6F5 && itemID <= 0x6F6)))
                        continue;
