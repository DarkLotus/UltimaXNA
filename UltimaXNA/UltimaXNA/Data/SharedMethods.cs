...
namespace UltimaXNA.Data
{
    class NativeMethods
    {
        ...
        public static T Read<T>(FileStream fs) where T : struct
        {
            byte[] buffer = new byte[Marshal.SizeOf(typeof(T))];
            fs.Read(buffer, (int)fs.Position, Marshal.SizeOf(typeof(T)));
            //ReadMemory(processHandle, address, Marshal.SizeOf(typeof(T)), out buffer);
        ...
        public static object RawDeserialize(byte[] rawData, int position, Type anyType)
        {
            ...
            object obj2 = new object();// = Marshal.PtrToStructure(destination, anyType);
            Marshal.PtrToStructure(destination, obj2);
